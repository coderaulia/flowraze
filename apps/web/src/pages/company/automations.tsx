...
function actionSummary(rule: AutomationRule) {
  if (rule.actionType === 'create_activity') {
    return `${rule.actionConfig.activityType?.replace('_', ' ') ?? 'Activity'}: ${rule.actionConfig.content ?? ''}`;
  }

  return `Set lead status to ${rule.actionConfig.status ?? 'selected status'}`;
}
...
export function AutomationsPage() {
  ...
  const handleCreateRule = async (event: React.FormEvent) => {
    ...
    const actionConfig = form.actionType === 'create_activity'
      ? { activityType: form.activityType, content: form.content }
      : { status: form.status };
  ...
  const handleManualRun = async (rule: AutomationRule) => {
    ...
    if (!leadId) {
      setMessage({ type: 'error', text: 'Lead ID is required for a manual run.' });
      return;
    }

    const response = await post(`/automations/${rule.id}/run`, { leadId });
    setMessage({
      type: response.success ? 'success' : 'error',
                  ...
                  <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                    <div className="min-w-0 space-y-1">
                      ...
                      <p className="text-sm text-on-surface-variant">
                        {rule.triggerEvent.replace('_', ' ')} {'->'} {actionSummary(rule)}
                      </p>
